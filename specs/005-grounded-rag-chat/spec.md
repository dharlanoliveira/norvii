...
A researcher asks a question that is not sufficiently supported by the active corpus and receives a clear abstention explaining that the available evidence is insufficient, with no fabricated answer or citation.

**Why this priority**: Safe refusal is a core quality behavior for legal research and prevents the POC from presenting plausible but unsupported statements as findings.

**Independent Test**: Ask a deterministic question absent from the corpus and verify that the response is an abstention, contains no unsupported factual claim, and offers a useful next action such as narrowing the question or reviewing available sources.

**Acceptance Scenarios**:

1. **Given** retrieval does not provide sufficient evidence for the question, **when** generation completes, **then** the chat returns a localized insufficient-evidence outcome instead of an answer.
2. **Given** all documents in the active corpus are pending or failed, **when** the researcher submits a question, **then** the chat explains that no ready evidence is available and does not call the answer path.
...
- **FR-004**: The system MUST produce an answer only when the retrieved evidence meets the configured minimum support threshold; otherwise it MUST abstain.
- **FR-005**: A completed answer MUST expose evidence references for the supporting passages, including corpus, source, document version, stable unit or text location, and the visible excerpt needed to inspect the support.
...
- **FR-007**: The chat MUST render answer output progressively while generation is active and MUST expose pending, streaming, completed, cancelled, abstained, and failed states.
...
- **FR-011**: Abstentions MUST state that the active corpus does not provide sufficient support and MUST NOT contain fabricated factual claims or unsupported citations.
...
- **Grounded answer**: A streamed or completed response whose supported statements are linked to retrieved evidence, or an explicit abstention when support is insufficient.
...
- **SC-004**: In deterministic unsupported-question scenarios, 100% of responses are explicit abstentions with zero fabricated citations or unsupported evidence references.
...
- **SC-006**: At least 95% of streamed responses show visible progress within 1 second of the first accepted generation event and never display a failed or cancelled response as completed.
