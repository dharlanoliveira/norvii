...
evidence-backed legal concepts and relationships that make graph retrieval possible. A successful
reingestion automatically stages a candidate snapshot, validates its graph release, and activates
the new snapshot only when the complete release is ready.
...
**Independent Test**: Reingest a changed source, verify that the prior active snapshot remains
available while publication is in progress, and verify that the new snapshot and graph release
become active together only after their evidence-backed projection is ready.
...
2. **Given** a source reingestion is still staging its candidate snapshot or graph release,
   **When** a researcher uses graph or hybrid retrieval, **Then** no candidate entity,
   relationship, or evidence location is returned.
3. **Given** a graph release cannot be built or validated, **When** a maintainer inspects the
   source processing state, **Then** Norvii reports a safe actionable failure diagnostic and
   preserves the preceding active snapshot and ready graph release.
4. **Given** a source reingestion completes successfully, **When** a researcher selects Vector,
   Graph, or Hybrid retrieval, **Then** all three strategies are ready for the newly active
   snapshot without a separate maintainer publication or graph-build action.
...
- **FR-003**: The system MUST create a graph release only from a staged immutable snapshot and
  keep the release identity, readiness state, and source snapshot identity inspectable.
- **FR-004**: The system MUST coordinate successful reingestion as canonical artifact
  publication, candidate snapshot staging, graph-release validation, and active-snapshot
  activation. A candidate revision MUST remain unavailable to all strategies until the graph
...
- **FR-016**: When immutable document content is reused during reingestion, the system MUST
  verify that its required semantic extraction artifacts exist before snapshot staging. A
  historical document without those artifacts MUST be enriched idempotently rather than leaving a
  ready source without graph-ready evidence.
...
- **SC-002**: In deterministic candidate and failed-projection journeys, 100% of candidate,
  stale, failed, or foreign graph releases are excluded from all retrieval strategies.
...
- **SC-008**: In the two seeded-corpus reingestion journeys, 100% of successful source attempts
  complete with a ready active snapshot and ready graph release without a separate operator
  command.
...
- A graph release may be unavailable only for a historical snapshot or after a failed staging
  attempt; the preceding active snapshot remains available rather than being downgraded.
...
- **Out of scope**: MCP tools and reusable skills, end-user graph authoring, cross-corpus graph
  traversal, free-form ontology expansion, a broad evaluation
