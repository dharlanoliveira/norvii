...
- **Online retrieval**: Planning sees only available assertion predicates and compatible entity labels. It decides graph use solely from whether the available assertion or hierarchy structure can materially improve an answer, never from whether vector evidence appears sufficient. A hierarchy scope matches a legal unit and its descendants, then returns the exact assertion and smallest ancestor path. All query values are parameterized and bounded.
