...
func TestCanonicalInitializationIsVectorCapableAndRepeatable(t *testing.T) {
	...
	if firstStatus.CurrentVersion != 1 || secondStatus.CurrentVersion != 1 {
		t.Fatalf("migration versions = %d and %d, want 1 and 1", firstStatus.CurrentVersion, secondStatus.CurrentVersion)
	}
