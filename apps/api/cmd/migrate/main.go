...
func run() int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "PostgreSQL migration initialization failed: %v\n", err)
		return 1
	}
	defer closeMigrator(migrator)

	status, err := persistence.NewMigrationService(migrator).Apply(ctx)
	if err != nil {
		fmt.Fprintf(os.Stderr, "PostgreSQL migration failed: %v\n", err)
		return 1
	}
